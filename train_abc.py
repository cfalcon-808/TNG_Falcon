...
#      (C) A multi-phase curriculum (list of flat dict phases) that continues
#          the same model across phases with a new TensorBoard subdir per phase
#
...
#        Suite name. Logs and models are grouped under this.
#    - CORE (matchup tag):
#        Generated by core_tag(learner_role, opponent_ai, mix=...)
#        Examples: GvNT, GvST, GvMixT, TvRG, TvMG
#    - VARIATIONS:
#        variation_name -> config, where config can be None for defaults,
#        a single dict for one phase, or list[dict] for a multi-phase curriculum
#    - Tiger mixing:
#        If mix_prob is set in a phase, each episode reset samples smart tiger
#        with probability mix_prob and greedy tiger otherwise
#    - Action masking:
#        Each worker env is wrapped as:
#        BaseEnv -> (Monitor or TigerMixWrapper or GoatMixWrapper) -> ActionMasker
#
#  Outputs:
#    artifacts/{EXPERIMENT_NAME}/
#      meta/         config, commit hash, and notes files
#      tb/           TensorBoard logs by phase
#      checkpoints/  periodic checkpoints per phase
#      models/       final and best model exports
#      eval/         evaluation outputs
#      logs/         auxiliary logs
#
#  Resume support:
#    RESUME_MODEL_PATH can point to a .zip file or a directory. If it points
#    to a directory, the newest .zip is used.
#
#  TensorBoard metrics:
#    - 1.episode_stats/* for cumulative outcome rates
#    - 2.window_stats/* for rolling outcome rates and realized opponent mix
#    - 3.matchup_info/* for learner and opponent metadata
#
#  Quick use:
#    1) Choose opponent and role
#    2) Set experiment identity and hardware
#    3) Define VARIATIONS
#    4) Run: python train_abc.py
#    5) Watch training: tensorboard --logdir artifacts --port 6006
#
#  Workflow:
#    1) Establish a baseline
#    2) Run one-at-a-time ablations
#    3) Run grouped ablations
#    4) Sweep selected reward magnitudes
#    5) Run curriculum and mixing experiments
#    6) Evaluate winners with eval_abc.py
#
# ============================================================
