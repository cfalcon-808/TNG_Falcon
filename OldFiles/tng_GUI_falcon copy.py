...
class TigersGoatsGUI:
    def __init__(self, tiger_ai: str, model_path=None, replay_path=None):
        ...
        self.canvas = tk.Canvas(self.root, width=600, height=420, bg="#222222", highlightthickness=0)
        self.canvas.grid(row=2, column=0, padx=10, pady=(10, 0))
        self.model_name_canvas = self.canvas.create_text(
            300, 415, text=self.model_name_var.get(), fill="white", anchor="s"
        )
    ...
    def _draw_static(self):
        ...
        radius = 22
        for idx, (x, y) in NODE_LAYOUT.items():
            circle = self.canvas.create_oval(
                x - radius, y - radius, x + radius, y + radius,
                fill="#444444", outline="#888888", width=2
            )
            label = self.canvas.create_text(x, y, text=str(idx), fill="white")
    ...
    def _loc_to_node(self, x, y):
        for idx, (nx, ny) in NODE_LAYOUT.items():
