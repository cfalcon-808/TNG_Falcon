...
#
#  Overview:
#    - Goat vs smart_tiger heuristic opponent (capture/anchor logic).
#    - Shares knob/weight tuning with the normal env via DEFAULT_KNOBS/DEFAULT_WEIGHTS and self._w.
#    - Includes action masking, repetition tracking, and human-readable move descriptions.
#    - Compatible with FlattenBattleActionWrapper, experiment_sweep, and eval_falcon.
#
#  Quick use:
#    env = BattleEnv()                                # baseline
#    env = BattleEnv(reward_weights={"bubble": 0.0})  # ablate bubble shaping
#    env = BattleEnv(reward_weights={"REWARD_GOAT_WIN": 5.0})  # tweak knob
#    env = BattleEnv(reward_weights={"BASE_TIGER_CAPTURE_BIAS": 0.7})  # adjust tiger bias
# ============================================================
...
DIR_CODES                 = 5           # direction codes per position (0–4)
TOTAL_GOATS_TO_PLACE      = 15          # goats placed before moving phase begins
GOATS_EATEN_FOR_TIGER_WIN = 6           # tiger win after this many captures
...
class BattleEnv(gym.Env):
    ...
    def __init__(self, reward_weights = None):
        ...
        self.phase = 0          # 0 = placing, 1 = moving
        self.goats_placed = 0
    ...
    # --------------------------------------------------------
    #  Weight/knob accessor (matches normal env)
    # --------------------------------------------------------
    def _w(self, key: str) -> float:
        ...
        raise KeyError(
            f"[BattleEnv] Unknown tuning key '{key}'. "
    ...
    # --------------------------------------------------------
    #  Gym API: reset (Matches normal env)
    # --------------------------------------------------------
    ...
    def step(self, action):
        """
        Execute one Goat + Tiger turn with shaping:
          1) Decode action (supports flat or MultiDiscrete)
          2) Goat place/move with invalid handling
          3) Recompute shaping signals (mobility, bubble, cluster, center, near-lock)
          4) Tiger acts via smart_tiger policy (capture/anchors/roam)
          5) Apply terminal checks (wins, timeouts, repeats) and update masks

        Returns: obs, reward, terminated, truncated, info(action_mask, winner?, invalid_action?)
        """
        ...
        if self.phase == 0:
            ...
            step_pen = (
                ...
                + self._w("MOVE_STEP_SLOPE") * self.move_steps
            )
    ...
    def _tiger_moves(self):
        ...
        for i in range(BOARD_SIZE): # tiger at this position
            if self.board[i] == 2:  
